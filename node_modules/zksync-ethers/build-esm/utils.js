 ...
 * @constant
 */
export const CONTRACT_2_FACTORY_ADDRESS = '0x0000000000000000000000000000000000010000';
/**
...
export function isETH(token) {
    return (isAddressEq(token, LEGACY_ETH_ADDRESS) ||
 ...
 */
export function getHashedL2ToL1Msg(sender, msg, txNumberInBlock) {
 ...
 */
export function getDeployedContracts(receipt) {
 ...
 */
export function create2Address(sender, bytecodeHash, salt, input = '') {
 ...
 */
export function createAddress(sender, senderNonce) {
 ...
 * }
 */
export async function checkBaseCost(baseCost, value) {
 ...
 */
export function serializeEip712(transaction, signature) {
 ...
 * *\/
 */
export function parseEip712(payload) {
    ...
    const raw = ethers.decodeRlp(bytes.slice(1));
    const transaction = {
        type: EIP712_TX_TYPE,
    ...
    if ((ethers.hexlify(ethSignature.r) === '0x' ||
...
export async function estimateDefaultBridgeDepositL2Gas(providerL1, providerL2, token, amount, to, from, gasPerPubdataByte) {
    ...
    if (await providerL2.isBaseToken(token)) {
        ...
        return await estimateCustomBridgeDepositL2Gas(providerL2, l1BridgeAddress, l2BridgeAddress, isAddressEq(token, LEGACY_ETH_ADDRESS) ? ETH_ADDRESS_IN_CONTRACTS : token, amount, to, bridgeData, from, gasPerPubdataByte, value);
    }
}
/**
 ...
 * // gas = 683_830
 */
export async function estimateCustomBridgeDepositL2Gas(providerL2, l1BridgeAddress, l2BridgeAddress, token, amount, to, bridgeData, from, gasPerPubdataByte, l2Value) {
 ...
 * // true
 */
export function isAddressEq(a, b) {
